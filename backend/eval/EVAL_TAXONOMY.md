...
versus what's still open. Multi-turn doesn't vary by phrasing — it's a
structural setup (a two-turn exchange), not a phrasing variant, so it's
marked N/A rather than left as a gap.

| Capability | Short | Long | Multi-intent | Ambiguous |
|---|---|---|---|---|
| Factual lookup | ✓ | open | ✓ | ✓ |
| Synthesis | ✓ | open | open | open |
| Amap API | ✓ | open | open | open |
| Chit-chat | ✓ | open | open | open |
| Knowledge base boundary | ✓ | open | open | open |
| Adversarial | ✓ | open | open | open |
...
- **Written so far: 9** (8 in the grid + Multi-turn's single case)
- **Still open: 16**
