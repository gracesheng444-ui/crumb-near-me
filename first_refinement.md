...
3. **Chit-chat over-elaboration** — add an explicit brevity/scope constraint for casual turns ("1-2 sentences, no unprompted recommendations, no invented specifics of any kind"), and test whether a lower `max_tokens` cap specifically for detected chit-chat forces brevity structurally rather than just by instruction.

4. **Judge reliability** — either accept the same-family limitation as a documented tradeoff (cheapest), or have the judge quote the specific fact it's disputing before scoring, forcing it to ground its own criticism rather than assert freely.
