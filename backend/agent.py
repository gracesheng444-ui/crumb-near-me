...
a retrieve_info result or a web_search result.
...
replying in. Web search results are different: they are NOT curated, so \
always tell the visitor the fact came from a web search (briefly, e.g. \
"according to a search, ..." / "根据网上的信息，...") rather than presenting it \
with the same certainty as a knowledge-base fact, and mention the source \
site by name when one is available.
